  ...
  <section class="h-screen flex items-center justify-center bg-gray-200">
    <div class="w-full max-w-3xl lg:max-w-7xl flex flex-col lg:flex-row items-center justify-center space-y-6 lg:space-y-0 lg:space-x-6">
      <div class="w-full lg:w-1/2 p-8 bg-white rounded-xl shadow-xl">
        <h2 class="text-center text-3xl font-bold mb-8">Регистрация организаций</h2>
        <form @submit.prevent="registerorg">
          <div class="mb-6">
            <label for="nameorganization" class="block text-lg font-medium mb-3">Имя организации</label>
            <input
              type="text"
              id="nameorganization"
              ...
              placeholder="Введите имя органицзации"
              required
            />
          </div>
          <div class="mb-6">
  ...
  methods: {
    async registerorg() {
      try {
        ...
        const response = await axios.post('http://127.0.0.1:8000/auth/register/org/', {
